...
use Kanekescom\Siasn\Api\Referensi\Transformers\RefJenjangJfTransformer;

class RefJenjangJf extends ClassExtender
{
    /**
    ...
    {
        $response = Referensi::getRefJenjangJf($params);
        ...
        if ($response->ok() && is_array($data)) {
            $items = fractal($data, new RefJenjangJfTransformer)->toArray();
