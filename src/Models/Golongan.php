...
use Kanekescom\Siasn\Api\Referensi\Transformers\GolonganTransformer;

class Golongan extends ClassExtender
{
    /**
    ...
    {
        $response = Referensi::getGolongan($params);
        ...
        if ($response->ok() && is_array($data)) {
            $items = fractal($data, new GolonganTransformer)->toArray();
