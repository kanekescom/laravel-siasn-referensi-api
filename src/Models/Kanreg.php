...
use Kanekescom\Siasn\Api\Referensi\Transformers\KanregTransformer;

class Kanreg extends ClassExtender
{
    /**
    ...
    {
        $response = Referensi::getKanreg($params);
        ...
        if ($response->ok() && is_array($data)) {
            $items = fractal($data, new KanregTransformer)->toArray();
